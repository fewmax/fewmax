...
import warnings
import collections
from data.visda_fewshot import VisDA17
import torch
...
from builder_4 import Builder # contrastive learner
...
parser.add_argument('--dataset', default='imagenet', type=str, choices=['imagenet', 'imagenet_val', 'cifar10', 'cifar100', 'speech_commands', 'covtype', 'higgs', 'higgsall', 'higgs100K', 'higgs100Kall', 'higgs1M', 'higgs1Mall', 'visda'],
                    help='dataset to use')
...
parser.add_argument('--method', default='moco', type=str, choices=['MaxUP', 'npair', 'CSG', 'moco', 'mocon', 'byol'],
                    help='method to use')
...
parser.add_argument('--pinv', action='store_true',
                    help='pinv')
parser.add_argument('--chunks', dest='chunks', default='1',
                    help='stage-wise chunk to feature maps, seperated by dot')
parser.add_argument('--csg-stages', dest='csg_stages', default='4',
                    help='resnet stages to involve in CSG, 0~4, seperated by dot')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = Builder(
        ...
        dim=args.dim, pred=args.pred, method=args.method, shuffle_bn=(args.bn == 'shuffle'), head_mul=args.head_mul, sym=args.sym, in_channels=in_channels, small=small, distributed=args.distributed, stages=args.stages, get_head=get_head, qlen=args.qlen)
    # print(model)
    ...
    if args.distributed:
        ...
        # raise NotImplementedError("Only DistributedDataParallel is supported.")rallel after resume
    ...
    if args.dataset in ['visda', 'visda_val']:
        ...
        train_dataset = VisDA17(txt_file=os.path.join(args.data, "train/image_list.txt"), root_dir=os.path.join(args.data, "train"),
                           transform=TwoCropsTransform(train_transform_list), portion=0.008)
    else:
...
def train(train_loader, model, criterion, optimizer, epoch, tb_logger, args):
    ...
    for i, (images, label) in enumerate(train_loader):
        ...
        # compute output
        m = None
        output, target, loss1, loss2, results  = model(im_1=images[0], im_2=images[1], m=m, criterion=criterion, imix=args.imix, alpha=args.alpha, mix_layers=args.mix_layers, num_aux=args.inputmix, alpha2=args.alpha2)
...
if __name__ == '__main__':
    ...
    if args.lincls:
        ...
        if args.dataset in ['imagenet', 'visda']:
            lin_lrs = ['30']
            lin_schedule = ['--lrd', '0.1', '--schedule', '8', '9', '--epochs', '10']
            aux = ''
        else:
        ...
        for target_dataset in target_datasets:
            for lin_lr in lin_lrs:
                ...
                args_lincls_list = [args.data, '-a', args.arch, '-j', str(args.workers), '--dataset', target_dataset, '--start-eval', '1', '--lr', lin_lr, '--trial', trial_str]
