...
class Builder(nn.Module):
    ...
    def __init__(self, base_encoder, dim=128,  K=65536, m=0.999, T=0.07, qlen=4096, emam=0.999, temp=0.2,  mlp=True, proj='lin', pred='none', method='npair', shuffle_bn=False, head_mul=1, sym=False, in_channels=3,
        ...
        self.encoder_q = base_encoder(
            num_classes=dim, pretrained=True, kaiming_init=kaiming_init, **base_encoder_kwargs)  # ######
        ...
        if type(self.encoder_q).__name__ == "ResNet":
            try:
                # BottleNeck
                dims = [self.encoder_q.conv1.weight.size()[0]] + [getattr(self.encoder_q,
                                                                          "layer%d" % stage)[-1].conv3.weight.size()[0] for stage in range(1, 5)]
            except:
                # BasicBlock
                dims = [self.encoder_q.conv1.weight.size()[0]] + [getattr(self.encoder_q,
                                                                          "layer%d" % stage)[-1].conv2.weight.size()[0] for stage in range(1, 5)]
        ...
        for stage in stages:
            ...
            setattr(self, "queue%d" % (stage), nn.functional.normalize(
                getattr(self, "queue%d" % (stage)), dim=0))
            self.register_buffer("queue_ptr%d" %
    ...
    def _dequeue_and_enqueue(self, keys, stage):
        ...
        if ptr + batch_size <= self.K:
            getattr(self, "queue%d" % (stage))[
                :, ptr:ptr + batch_size] = keys.T
        else:
            getattr(self, "queue%d" % (stage))[
                :, ptr:] = keys[:(self.K - ptr)].T
            getattr(self, "queue%d" % (stage))[
                :, :ptr + batch_size - self.K] = keys[:(ptr + batch_size - self.K)].T
    ...
    def forward(self, im_1, im_2, m=None, criterion=None, imix='none', alpha=1., mix_layers='0', num_aux=0, alpha2=1.):
        ...
        if self.method == 'MaxUP':
            ...
            for s, (im_q, im_k) in enumerate(im_qk):
                ...
                #im_q = torch.cat([im_q, im_q_mix_0, im_q_mix_1, im_k], dim=0)
                q, features_new = self.encoder_q(im_q, output_features=[
                                                 "layer%d" % stage for stage in self.stages], task=self.task)  # queries: NxC

                q = nn.functional.normalize(q, dim=1)

                # compute key features
                q, q_0, q_1, k = q[:bsz], q[bsz:int(
                    2*bsz)], q[int(2*bsz):int(3*bsz)], q[int(3*bsz):]
                #logits = q.mm(k.t())
                logits_0 = q_0.mm(k.t())
                logits_1 = q_1.mm(k.t())
                ...
                with torch.no_grad():
                    ...
                    loss_0 = (lam_0 * criterion(logits_0, labels) +
                              (1. - lam_0) * criterion(logits_0, labels_aux_0)).mean()
                    loss_1 = (lam_1 * criterion(logits_1, labels) +
                              (1. - lam_1) * criterion(logits_1, labels_aux_1)).mean()

                    loss = max(loss_0, loss_1)

                if loss == loss_0:
                    glogits = logits_0
                    loss = (lam_0 * criterion(logits_0, labels) + (1. -
                            lam_0) * criterion(logits_0, labels_aux_0)).mean()
                elif loss == loss_1:
                    glogits = logits_1
                    loss = (lam_1 * criterion(logits_1, labels) + (1. -
                            lam_1) * criterion(logits_1, labels_aux_1)).mean()
                glabels = labels
                gloss = loss

        else:
            im_q, im_k = im_1, im_2
            input_ = torch.cat([im_q, im_k], dim=0)

            output_, features_new = self.encoder_q(input_, output_features=[
                                                   "layer%d" % stage for stage in self.stages], task=self.task)  # queries: NxC
            output_ = nn.functional.normalize(output_, dim=1)

            q, k = output_[:bsz], output_[bsz:]
            glogits = q.mm(k.t())

            glogits /= self.temp
            glabels = torch.arange(bsz, dtype=torch.long).cuda()
            gloss = criterion(glogits, glabels).mean()
        ...
        _, features_old = self.encoder_k.forward_backbone(
            im_k, output_features=["layer%d" % stage for stage in self.stages])
        ...
        for idx, stage in enumerate(self.stages):
            ...
            if self.mlp:
                q_ = self.encoder_q.fc_csg["stage%d" % (stage)](
                    self.encoder_q.avgpool(q_feature).view(q_feature.shape[0], -1))

            q_ = nn.functional.normalize(q_, dim=1)

            # compute key features
            with torch.no_grad():  # no gradient to keys
                k_feature = chunk_feature(
                    features_old["layer%d" % stage], chunk)
                # A-Pool #############
                if self.mlp:
                    k_ = self.encoder_k.fc_csg["stage%d" % (stage)](self.encoder_k.avgpool(
                        k_feature).view(features_old["layer%d" % stage].size(0)*chunk**2, -1))
                # #####################
            ...
            # negative logits: NxK
            l_neg = torch.einsum(
                'nc,ck->nk', [q_, getattr(self, "queue%d" % (stage)).clone().detach()])
            # logits: Nx(1+K)
            ...
            for idx in range(len(self.stages)):
                ...
                # targets: zeros
                csg_loss = CrossEntropyLoss(
                    results['predictions_csg'][idx], results['targets_csg'][idx])
...
def concat_all_gather(input):
    gathered = [torch.ones_like(input) for _ in range(
...
def cutmix(input, alpha):
    if not isinstance(alpha, (list, tuple)):
        alpha = [alpha, alpha]
    beta = torch.distributions.beta.Beta(torch.tensor([2.]), torch.tensor([20.]))
    randind = torch.randperm(input.shape[0], device=input.device)
    lam = beta.sample().to(device=input.device)
    lam = torch.max(lam, 1. - lam)
...
def rand_bbox(size, lam):
    ...
    new_lam = 1. - (bbx2 - bbx1).to(lam.dtype) * \
        (bby2 - bby1).to(lam.dtype) / (W * H)
